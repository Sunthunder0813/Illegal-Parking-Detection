...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "  - IP Camera:  'rtsp://192.168.18.2/' or 'rtsp://192.168.18.71/'\n"
...
def run_inference_pipeline(net, input, batch_size, labels, output_dir,
    ...
    # Initialize input source from string: "camera", video file, image folder, or RTSP stream.
...
def main() -> None:
    ...
    """
    if len(sys.argv) == 1:
        # No arguments: run both IP cameras in parallel
        ip_cameras = ["rtsp://192.168.18.2/", "rtsp://192.168.18.71/"]
        threads = []
        for ip in ip_cameras:
            def run_for_camera(ip_cam):
                args = parse_args()
                args.input = ip_cam
            ...
            t = threading.Thread(target=run_for_camera, args=(ip,))
            t.start()
            threads.append(t)
        for t in threads:
            t.join()
